    ...
    function displayVideoInfo(data) {
        ...
        if (data.thumbnail) {
            ...
            videoThumbnail.onerror = function () {
                // If thumbnail fails to load (CORS), use proxy fallback
                const proxyUrl = `https://images.weserv.nl/?url=${encodeURIComponent(data.thumbnail)}&w=480&output=jpg`;
                if (this.src !== proxyUrl) {
                    this.src = proxyUrl;
                } else {
                    this.hidden = true;
                }
